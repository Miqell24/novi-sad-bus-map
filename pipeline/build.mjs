...
// Novi Sad: ONE feed for JGSP Novi Sad — 101 city and suburban bus lines
// across Vojvodina's capital and the villages around it, from Beočin under
// Fruška Gora to Žabalj on the Tisa plain. No tram (the last one ran in 1958),
// no trolleybus.
...
// Exact-match repairs for poles a feed names badly. Novi Sad needs none —
// 1 011 stop names, not one of them shouted.
const NAME_FIX = {};
...
// ONE feed, ONE cfg.
//
// The feed is generated from JGSP's own timetables (Nedžad Beus's
// gtfs-generators on GitLab, the source Transitous uses for Serbia) and ships
// shapes. It publishes ONE ROUTE ROW PER PATTERN — 219 rows for 101 line
// numbers — so the rows merge on the shared key by themselves and the engine
// picks a representative pattern per direction, exactly as it does for the
// feeds that ship one row per line.
//
// direction_id is 0 on every trip, so the engine falls back to keying
// directions by headsign; this feed fills headsigns everywhere.
const LBL = new Map();
const isRailTrunk = () => false;

const MODES = [{
  mode: 'bus', label: 'buses', graphMode: 'road',
  // Novi Sad and its villages — 3 × 3 tiles
  osmFiles: Array.from({ length: 9 }, (_, i) => `data/osm/tiles/t${i + 1}.json`),
  color: '#0059a9', colorDark: '#00294f',
  all: busAll, lines: busList.length ? busList : (busAll ? [] : ['4']),
  feeds: [
    { tag: 'ns', dir: 'data/gtfs', routeTypes: ['3'], mapKey: (sn) => sn || null },
  ],
}];
...
await (await import('./night.mjs')).nightPass(outDir, /^\d+N[A-Z]?$/, { sort: true });
