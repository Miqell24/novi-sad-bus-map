...
Novi Sad: 3 x 3 road tiles — the suburban lines reach Beocin, Sremski
Karlovci and Zabalj.
"""
...
PBFS = [os.path.join(ROOT, 'data', 'serbia-latest.osm.pbf')]

# must match pipeline/download.sh
S, N, W, E = 45.09, 45.47, 19.49, 20.08
RAIL_BOX = None
...
for i in range(1, 10):
    ...
    if os.path.exists(f):
        continue
    row, col = (i - 1) // 3, (i - 1) % 3
    road_tiles[i] = (S + (N - S) * row / 3, S + (N - S) * (row + 1) / 3,
                     W + (E - W) * col / 3, W + (E - W) * (col + 1) / 3)
rail_file = None
...
GEN = 'pbf-tiles.py (Geofabrik serbia)'
